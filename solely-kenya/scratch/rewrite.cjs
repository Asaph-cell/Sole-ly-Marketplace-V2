...
const targetFile = path.resolve('c:\\Users\\Asaph Isweka\\Downloads\\Solely v2\\solely-kenya\\supabase\\functions\\_shared\\email-service.ts');
let content = fs.readFileSync(targetFile, 'utf8');

// Find the start of the emailTemplates export
const templatesStartIndex = content.indexOf('export const emailTemplates = {');
if (templatesStartIndex === -1) {
    console.error('Could not find emailTemplates');
    process.exit(1);
}

// Keep the top part (imports, normalizePhone, sendEmail)
const topPart = content.substring(0, templatesStartIndex);
...
const finalContent = topPart + baseEmailLayout + newTemplates;
fs.writeFileSync(targetFile, finalContent);
console.log('Successfully updated templates!');
